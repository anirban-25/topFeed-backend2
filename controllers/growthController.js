...
export async function processGrowth(req, res) {
  try {
    console.log("Starting processGrowth");
    ...
    const allResults = await Promise.all(
      usersSnapshot.docs.map(async (userDoc) => {
        ...
        return null; // Return null for non-Growth plans
      })
    );
    ...
    res.json({
      message: "Growth plan processing completed successfully",
      processedUsers: filteredResults.length,
      results: filteredResults,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error(`Error in processGrowth:`, error);
