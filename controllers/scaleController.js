...
export async function processScale(req, res) {
  try {
    console.log("Starting processScale");
    ...
    const allResults = await Promise.all(
      usersSnapshot.docs.map(async (userDoc) => {
        ...
        return null; // Return null for non-Scale plans
      })
    );
    ...
    res.json({
      message: "Scale plan processing completed successfully",
      processedUsers: filteredResults.length,
      results: filteredResults,
      timestamp: new Date().toISOString(),
    });
  } catch (error) {
    console.error(`Error in processScale:`, error);
