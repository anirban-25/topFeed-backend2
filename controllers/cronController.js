...
export async function processCron(req, res) {
  try {
    ...
    const allResults = await Promise.all(
      usersSnapshot.docs.map(async (userDoc) => {
        ...
        try {
          ...
          console.log(tweetFeedData.twitterUrls)
          ...
          for (const tweet of result) {
            ...
            // Send notification if the tweet meets the user's notification criteria
            if (notificationLevels.includes(tweet.relevancy.toLowerCase())) {
              await sendTelegramMessage(
                telegramUserId,
                `New relevant tweet: ${tweet.url}`
              );
            }
          }
    ...
    console.error(`Error in processCron:`, error);
