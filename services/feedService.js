...
async function fetchRssFeeds(urls, topic, userId) {
  ...
  // console.log(`Last published date: ${last_date}`);
    ...
    // Update the last_date in Firebase if we have new data
    // if (filteredData.length > 0) {
    //   const newLastDate = filteredData[0].date_published;
    //   await setDoc(userRef, { last_date: newLastDate }, { merge: true });
    // }
  
    // Use the filteredData for further processing or return it
    // return filteredData;
  
  
  // const currentDate = new Date();
  // const fifteenMinutesAgo = subHours(currentDate, 2);
  // const filteredData = twitterData
  ...
  try {
    const db = getFirestore();
    const userRef = doc(db, 'users', userId);
    await setDoc(userRef, { last_date: last_date.toISOString() }, { merge: true });
